app.controller('EditReportController', function ($scope, apiService, $timeout, $location, $routeParams) {
    ...
    //Month Select option
    $scope.months = [];
    for (let i = 1; i <= 12; i++) { $scope.months.push(i); };
    //Month Select option

    $scope.bool = true;
    $scope.Report = new Report(NaN, "title...",
        new SimpleDate($scope.date.getDay(), $scope.date.getMonth() + 1, $scope.date.getFullYear()),
        "Content to Edit/Create...");
    //Data-models

    //Load the $scope.Report with the relevent Report
    apiService.EditReport(<number>$routeParams.Id).then(
        function (response)
        {
            $scope.Report = new Report(response.data.Id, response.data.Title,
    ...
    {
        apiService.SaveChanges($scope.Report).then(
            function (response)
            {
                alert("successfully Updated Report");
                let index: number = response.data;
